...
/**---------------------------  EMVCO APIs -------------------------------------------------------*/
     ...
     //12.0.46
     int EmvPrePare55Field(out byte[] OutPut, int OutBufSize);
/*---------------------------  Express APIs -------------------------------------------------------*/
    //13.0.1
    ...
    //13.0.4
    int ExpressTerminalParamSet(String  jString);
    ...
    //13.0.9
    int ExpressTransaction( );

    //13.0.10
    int ExpressSetTransactionType(byte TransType);

    //13.0.11
    int  ExpressSetTransactionAmount(int  uiAmount, int  uiAmountOther);
...
/*---------------------------  other misc APIs -------------------------------------------------------*/
     ...
     int Lib_SetLed(byte ledn,byte mode);
      ...
      //12.0.31
      int EmvSetPinblockType(int Type, int keyindex);

     //12.0.32
      void EmvSetCurrencyCode( String code);

     //12.0.33
      void EmvSetKeyPadPrompt( String prompt);
    ...
    //Set EMV callback
     void EmvSetCallback(IEmvProcessCallback emvCallback);

    //Sets a custom keypad layout for PIN capture screen
    int KeyPad_setCustomLayout(in int[] positions);

    //Allows to get digits keys
    int KeyPad_getPinKeys(out byte[] keys);

    //set emv inputPin callback
    int EmvSetInputPinCallback(in int timeout, IInputPinCallback callback);

    //Request a Online PIN
    int EmvKernelPinInput(in int timeout, in int keyId, IInputPinCallback callback);

    //Allows to confirm the Pin response
    void EmvSetPinInputResponse(boolean confirm, boolean isBypassPin);

    void EmvDoOnlineOfflinePIN(boolean confirm);

    void EmvSetSelAppResponse(int confirm);

    int SleOpenAt24(byte slot);

    int SleCloseAt24(byte slot);

    int SleCheckAt24(byte slot);

    int SleReadMemAt24(byte slot, int startAddress, int length, out byte []outData);

    int SleWriteMemAt24(byte slot, int startAddress, int length, in byte []inData);
